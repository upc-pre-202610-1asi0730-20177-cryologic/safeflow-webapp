...
function submit() {
  error.value = ''
  try {
    if (remember.value && email.value.trim()) {
      localStorage.setItem('sf_remember_email', email.value.trim())
    } else {
  ...
  window.setTimeout(() => {
    ...
    const target =
      typeof next === 'string' && next.startsWith('/') && next !== '/login'
        ? next
        : '/analytics'
    ...
    <div class="sf-sign__inner">
      <div class="sf-sign__card">
        <header class="sf-sign__hero">
          ...
          <h1 class="sf-sign__hero-title">{{ t('auth.login.heroTitle') }}</h1>
        </header>

        <form class="sf-sign__form" @submit.prevent="submit">
          <div class="sf-sign__field">
            <label class="sf-sign__label" for="sf-login-email">{{ t('auth.login.email') }}</label>
            <input
              id="sf-login-email"
              v-model="email"
              type="email"
          ...
          <div class="sf-sign__field">
            <label class="sf-sign__label" for="sf-login-password">{{ t('auth.login.password') }}</label>
            <input
          ...
          <div class="sf-sign__row">
            <div class="sf-sign__remember">
              <input
                id="sf-login-remember"
                v-model="remember"
                type="checkbox"
                class="sf-sign__checkbox"
              />
              <label class="sf-sign__remember-label" for="sf-login-remember">{{
                t('auth.login.remember')
              }}</label>
            </div>
            <a href="#" class="sf-sign__forgot" @click.prevent>{{ t('auth.login.forgot') }}</a>
          </div>
        ...
        <div class="sf-sign__divider">
          ...
          <span class="sf-sign__divider-text">{{ t('auth.login.divider') }}</span>
          <span class="sf-sign__divider-line" />
        </div>

        <RouterLink to="/register" class="sf-sign__secondary" role="button">
